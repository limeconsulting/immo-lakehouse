...
This project ingests French DVF (Demandes de Valeurs Foncières) data, builds a Lakehouse architecture using Iceberg, computes aggregated real-estate metrics and exposes them in ClickHouse for analytics.
...
Displays the pipeline scope before execution.
...
These provide aggregated price metrics per commune.
...
If Iceberg metadata is deleted while Nessie still references the table, the catalog may become inconsistent.
