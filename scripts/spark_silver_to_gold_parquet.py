...
from pyspark.sql.functions import col, month, expr, percentile_approx
...
def main():
    ...
    ap.add_argument(
        "--gold-prefix",
        default="gold/m_price_m2_commune_month",
        help="Path prefix under bucket (default: gold/m_price_m2_commune_month)",
    )
    args = ap.parse_args()
    ...
    spark = (
        SparkSession.builder.appName("dvf-silver-to-gold")
    ...
    df = spark.table("nessie.immo.dvf_silver")

    # Pin to ONE ingest-id + scope (this is the whole point of "clean ingest-all")
    df = (
        df.where(col("ingest_id") == args.ingest_id)
          .where(col("dep") == args.dep)
          .where(col("annee") == args.year)
    )

    # Dedup fix (keep). Add ingest_id to the key is unnecessary since we already filtered,
    # but leaving it out makes the intent clearer.
    ...
    df = (
        df.where(col("valeur_fonciere").isNotNull())
          .where(col("surface_reelle_bati").isNotNull())
          .where(col("surface_reelle_bati") > 0)
    ...
    agg = df.groupBy("code_commune", "nom_commune", "annee", "mois", "type_local").agg(
        ...
        percentile_approx("prix_m2", 0.75, 200).alias("prix_m2_p75"),
    )

    # Gold output path: versioned by ingest_id (recommended)
    out = (
        f"s3a://{args.bucket}/"
        f"{args.gold_prefix}/"
        f"year={args.year}/dep={args.dep}"
    )

    agg.write.mode("overwrite").parquet(out)

    print(f"OK: wrote gold parquet to {out}")
    spark.stop()
